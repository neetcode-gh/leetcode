...
class Solution {
    ...
    findShortestWay(maze, ball, hole) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (const [nextRow, nextCol, dist, direction] of getNeighbors(
                row,
                col,
            )) {
                pq.enqueue({
...
impl Solution {
    pub fn find_shortest_way(maze: Vec<Vec<i32>>, ball: Vec<i32>, hole: Vec<i32>) -> String {
        ...
        let valid = |r: i32, c: i32| -> bool {
            r >= 0 && c >= 0 && (r as usize) < m && (c as usize) < n
                && maze[r as usize][c as usize] == 0
        };

        let directions: [(i32, i32, &str); 4] = [
            (0, -1, "l"), (-1, 0, "u"), (0, 1, "r"), (1, 0, "d"),
        ];

        // Min-heap: (distance, path, row, col)
        let mut heap = BinaryHeap::new();
        heap.push(Reverse((0i32, String::new(), ball[0] as usize, ball[1] as usize)));
        let mut seen = vec![vec![false; n]; m];

        while let Some(Reverse((dist, path, row, col))) = heap.pop() {
            if seen[row][col] {
                continue;
            }
            if row == hr && col == hc {
            ...
            for &(dy, dx, dir) in &directions {
                let (mut cr, mut cc) = (row as i32, col as i32);
                let mut d = 0;
                while valid(cr + dy, cc + dx) {
                    cr += dy;
                    cc += dx;
                    d += 1;
                    if cr as usize == hr && cc as usize == hc {
                        break;
                    }
                }
                let mut new_path = path.clone();
                new_path.push_str(dir);
                heap.push(Reverse((dist + d, new_path, cr as usize, cc as usize)));
            }
        }
