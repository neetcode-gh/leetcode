...
class Solution {
    ...
    assignBikes(workers, bikes) {
        const findDistance = (workerLoc, bikeLoc) => {
            return (
                Math.abs(workerLoc[0] - bikeLoc[0]) +
                Math.abs(workerLoc[1] - bikeLoc[1])
            );
        };
...
impl Solution {
    pub fn assign_bikes(workers: Vec<Vec<i32>>, bikes: Vec<Vec<i32>>) -> Vec<i32> {
        let find_distance = |worker: &Vec<i32>, bike: &Vec<i32>| -> i32 {
            (worker[0] - bike[0]).abs() + (worker[1] - bike[1]).abs()
        };

        let mut all_triplets: Vec<(i32, usize, usize)> = Vec::new();
        for worker in 0..workers.len() {
            for bike in 0..bikes.len() {
                let distance = find_distance(&workers[worker], &bikes[bike]);
                all_triplets.push((distance, worker, bike));
            }
        }

        all_triplets.sort();

        let mut bike_status = vec![false; bikes.len()];
        let mut worker_status = vec![-1i32; workers.len()];
        let mut pair_count = 0;

        for &(_, worker, bike) in &all_triplets {
            if worker_status[worker] == -1 && !bike_status[bike] {
                bike_status[bike] = true;
                worker_status[worker] = bike as i32;
                pair_count += 1;
                if pair_count == workers.len() {
                    return worker_status;
                }
            }
        }
...
impl Solution {
    pub fn assign_bikes(workers: Vec<Vec<i32>>, bikes: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        let mut dist_to_pairs: HashMap<i32, Vec<(usize, usize)>> = HashMap::new();

        for worker in 0..workers.len() {
            for bike in 0..bikes.len() {
                let distance = find_distance(&workers[worker], &bikes[bike]);
                dist_to_pairs.entry(distance).or_default().push((worker, bike));
                min_dist = min_dist.min(distance);
            }
        }

        let mut curr_dist = min_dist;
        let mut bike_status = vec![false; bikes.len()];
        let mut worker_status = vec![-1i32; workers.len()];
        let mut pair_count = 0;

        while pair_count < workers.len() {
            if let Some(pairs) = dist_to_pairs.get(&curr_dist) {
...
impl Solution {
    pub fn assign_bikes(workers: Vec<Vec<i32>>, bikes: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        let mut worker_to_bike_list: Vec<Vec<(i32, usize, usize)>> = Vec::new();
        let mut pq = BinaryHeap::new();

        for worker in 0..workers.len() {
            let mut curr_worker_pairs: Vec<(i32, usize, usize)> = Vec::new();
            for bike in 0..bikes.len() {
                let distance = find_distance(&workers[worker], &bikes[bike]);
                curr_worker_pairs.push((distance, worker, bike));
            }
            curr_worker_pairs.sort_by(|a, b| b.cmp(a));
            let closest = curr_worker_pairs.pop().unwrap();
            pq.push(std::cmp::Reverse(closest));
            worker_to_bike_list.push(curr_worker_pairs);
        }

        let mut bike_status = vec![false; bikes.len()];
        let mut worker_status = vec![-1i32; workers.len()];

        while let Some(std::cmp::Reverse((_, worker, bike))) = pq.pop() {
            if !bike_status[bike] {
                bike_status[bike] = true;
                worker_status[worker] = bike as i32;
            } else {
                let next_closest = worker_to_bike_list[worker].pop().unwrap();
                pq.push(std::cmp::Reverse(next_closest));
            }
        }
