...
impl Solution {
    pub fn longest_consecutive(root: Option<Rc<RefCell<TreeNode>>>) -> i32 {
        let mut maxval = 0;
        Self::longest_path(&root, &mut maxval);
        maxval
    }

    fn longest_path(root: &Option<Rc<RefCell<TreeNode>>>, maxval: &mut i32) -> (i32, i32) {
        let node = match root {
            None => return (0, 0),
            Some(n) => n.borrow(),
        };
        ...
        if let Some(ref left) = node.left {
            let (li, ld) = Self::longest_path(&node.left, maxval);
            let left_val = left.borrow().val;
            if node.val == left_val + 1 {
                dcr = ld + 1;
            } else if node.val == left_val - 1 {
                inr = li + 1;
            }
        }

        if let Some(ref right) = node.right {
            let (ri, rd) = Self::longest_path(&node.right, maxval);
            let right_val = right.borrow().val;
            if node.val == right_val + 1 {
                dcr = dcr.max(rd + 1);
            } else if node.val == right_val - 1 {
                inr = inr.max(ri + 1);
            }
        }

        *maxval = (*maxval).max(dcr + inr - 1);
        (inr, dcr)
    }
}
```
