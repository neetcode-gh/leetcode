    ...
    findRotateSteps(ring, key) {
        ...
        for (let k = m - 1; k >= 0; k--) {
            for (let r = 0; r < n; r++) {
                for (let i = 0; i < n; i++) {
                    if (ring[i] === key[k]) {
                        const minDist = Math.min(Math.abs(r - i), n - Math.abs(r - i));
                        dp[k][r] = Math.min(dp[k][r], minDist + 1 + dp[k + 1][i]);
                    }
                }
            }
...
class Solution {
    ...
    findRotateSteps(ring, key) {
        const n = ring.length;
        const m = key.length;
        const dp = Array(n).fill(0).map((_, i) => Math.min(i, n - i));
        ...
        return Math.min(...adj[key.charCodeAt(m - 1) - 97].map(i => dp[i])) + m;
    }
}
```
