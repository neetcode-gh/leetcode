    ...
    groupStrings(strings) {
        function getHash(string) {
            ...
            for (let i = 0; i < string.length - 1; i++) {
                const diff =
                    (string.charCodeAt(i + 1) - string.charCodeAt(i) + 26) % 26;
    ...
    pub fn group_strings(strings: Vec<String>) -> Vec<Vec<String>> {
        fn get_hash(s: &str) -> String {
            ...
            for i in 1..bytes.len() {
                let diff = ((bytes[i] as i32 - bytes[i - 1] as i32 + 26) % 26) as u8 + b'a';
                hash_key.push(diff as char);
            }
            hash_key
        }
        ...
        for s in strings {
            let hash_key = get_hash(&s);
            map.entry(hash_key).or_default().push(s);
        }
